

function Haiku1(){
    console.log("haiku by:","Felix Huiskamp");
    console.log("My life");
    console.log("How much more of it remains?");
    console.log("The night is brief.");
}
function Haiku2(){
        console.log("haiku by:","Reik de Vries");{
        console.log("haiku by:","Docent");
        console.log("Panda in the mist");
        console.log("Gently chewing on bamboo");
        console.log("Perfect breakfast time");
    
}
function Haiku3(){
    console.log("haiku by:","Amir");
    console.log("The Taste");
    console.log("of rain");
    console.log("Why kneel");
}
function Haiku4(){
    console.log("haiku by:","isaiah");
    console.log("i love banaan");
    console.log("monkey lover");
    console.log("never have i ever");
}


Haiku1()
Haiku2()
Haiku3()
Haiku4()